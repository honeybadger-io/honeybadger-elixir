defmodule Honeybadger.HTTPAdapter.Httpc do
  @moduledoc """
  HTTP adapter module for making http requests with `:httpc`.

  SSL support will be enabled automatically if the `:certifi` and
  `:ssl_verify_fun` libraries exist in your project. You can also
  override the `:httpc` options by updating the configuration:

      http_adapter: {Honeybadger.HTTPAdapter.Httpc, [...]}

  For releases please make sure you have included `:inets` in your
  application:
  ...
  defp set_content_length_header(headers, body) do
    case List.keyfind(headers, ~c"content-length", 0) do
      nil ->
        length = body |> IO.iodata_length() |> Integer.to_string() |> to_charlist()
  ...
  defp raise_on_missing_ssl_verify_fun! do
    Code.ensure_loaded?(:ssl_verify_hostname) ||
      raise """
      This request's SSL certificate cannot be verified.

      Please add `:ssl_verify_fun` to your project dependencies:
  ...
  defp raise_on_missing_certifi! do
    Code.ensure_loaded?(:certifi) ||
      ...
      Please add `:certifi` to your project dependencies:
